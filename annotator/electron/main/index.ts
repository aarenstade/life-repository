...
const initializeDirectories = () => {
  const ROOT_DIR = path.join(app.getPath("home"), ".novella");
...
const initializeConfig = () => {
  const configPath = path.join(process.env.ROOT_DIR || "", "config.json");
...
const initializeDatabase = async () => {
  const schemaPath = path.join(process.env.ROOT_DIR || "", "schema.sql");
  const db = new LocalDatabaseAdapter(process.env.DATABASE_PATH || "");
  db.createSchema(schemaPath);
};
