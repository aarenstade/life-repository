...
if __name__ == "__main__":
    ...
    metadata = get_image_properties(image_path)
    print(f"Metadata properties for {image_path}:\n{metadata}\n")
    try:
        latitude, longitude = get_image_location(image_path)
        print(f"Location for {image_path}:\n{latitude}, {longitude}")
    except ValueError:
        print(f"No location data found for {image_path}")

    xmp = get_image_xmp_metadata(image_path)
    print(f"Metadata XMP tags for {image_path}:\n{xmp}")
